...
export const solveGalileo = ({ g, alpfa, v0, }) => v0 * v0 * Math.sin(2 * alpfa) / g;
export const solveGalileoHeight = ({ g, alpfa, v0, }) => (v0 * Math.sin(alpfa)) ** 2 / (2 * g);
export const solveIterate = ({ g, alpfa, v0, r, n, m, cf, s, }) => {
    const k = r * cf * s / (2 * m);
...
export const solveRungeKutta = ({ v0, alpfa, n, r, s, cf, m, g, }) => {
    const k = r * cf * s / (2 * m);
    const Ts = 0;
    const Te = 1000;
    let Y = [0, 0, v0 * Math.cos(alpfa), v0 * Math.sin(alpfa)];
    const ff = (t, [x, y, vx, vy]) => [
        vx,
        vy,
...
export const solveRungeKuttaHeight = ({ v0, alpfa, n, r, s, cf, m, g, }) => {
    const k = r * cf * s / (2 * m);
    const Ts = 0;
    const Te = 1000;
    let Y = [0, 0, v0 * Math.cos(alpfa), v0 * Math.sin(alpfa)];
