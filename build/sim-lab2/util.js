...
const getNeighborTrianglesPoints = (t) => {
    const [p1, p2, p3] = t.points;
    return [
...
const getThirdPointFromOtherTriangle = (p1, p2, t) => getThirdPoint(p1, p2, findOtherTriangle(p1, p2, t));
const isNeighborTriangles = (t1, t2) => {
...
export const initDelaunay = async (config) => {
    ...
    const trianglesFromLastCheck = new Set();
    const data = {
        ...
        async getIncorrectTriangles() {
            Array.from(trianglesFromLastCheck).forEach((triangle) => {
                getNeighborTriangles(triangle).forEach((neighborTriangle) => trianglesFromLastCheck.add(neighborTriangle));
            });
            const triangles = Array.from(trianglesFromLastCheck).filter((triangle) => getNeighborTrianglesPoints(triangle).some((point) => !checkDelaunay(triangle, point)));
...
const iterativeDelaunay = async (data, newPoint) => {
    ...
    switch (lastType) {
        ...
        case 'v1-v3':
        case 'v2-v3': {
            const i1 = lastType[1] - 1;
            const i2 = lastType[4] - 1;
         ...
         */
        case 'out':
        default: throw new Error(`incorrect lastType ${JSON.stringify(lastType)}`);
    }
    /**
