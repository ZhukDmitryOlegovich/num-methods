import { SquareMatrix } from '../math/index.js';
...
6 3`;
    ...
    button2.onclick = () => {
        textarea.value = '';
        textarea.value += `${fromLength(currentSize, (j) => fromLength(currentSize, () => (Math.random() * (inputB.valueAsNumber - inputA.valueAsNumber) + inputA.valueAsNumber)).join(' ')).join('\n')}\n\n`;
    };
    ...
    button.onclick = () => {
        ...
        const arrNumbers = fromLength(currentSize, (i) => numbers.slice(i * currentSize, (i + 1) * currentSize));
        const a = SquareMatrix.fromMatrix(new SquareMatrix(arrNumbers).transpose());
        const { L, U } = a.decomposeLU();
        p.innerHTML = '';
        const print = (m) => m.transpose().matrix.map((e) => e.join(' ')).join('<br>');
        [
            '<b>= L</b>',
            print(L),
            '',
