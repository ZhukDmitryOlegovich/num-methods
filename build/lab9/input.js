...
1 1 1`;
    ...
    button2.onclick = () => {
        textarea.value = '';
        const m = fromLength(currentSize, (j) => fromLength(currentSize, (i) => (Math.random() * (inputB.valueAsNumber - inputA.valueAsNumber) + inputA.valueAsNumber) * (i === j ? inputC.valueAsNumber : 1)));
    ...
    button.onclick = () => {
        ...
        const data = [];
        const maxCount = 1000;
        const eigs = window.math.eigs(a.matrix).values;
        const [minL, maxL] = eigs.reduce(([min, max], v) => [v < min ? v : min, v > max ? v : max], [Infinity, -Infinity]);
