...
(() => {
    ...
    const mu = (A) => A.normaMax() * A.inverse().normaMax();
    const N = 5;
    button1.onclick = () => {
        ...
        let counter = 0;
        const AA = [];
        fromLength(countTest, () => new SquareMatrix(fromLength(N, () => fromLength(N, () => random(0, 10))))).forEach((A) => {
            var _a;
            const muA = mu(A);
            const f = A.mul(x);
            const xx = muA;
