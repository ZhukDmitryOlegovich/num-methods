...
(() => {
    ...
    button2.onclick = () => {
            ...
            for (let isDiag = 0; isDiag <= 1; isDiag++) {
                ...
                for (let smartColon = 0; smartColon <= 1; smartColon++) {
                    for (let smartRow = 0; smartRow <= 1; smartRow++) {
                        const vX2 = mA.eliminationGaussian(vB, { smartColon: !!smartColon, smartRow: !!smartRow });
                        const diff = Vector.fromMatrix(vX.add(vX2.mulN(-1))).norma();
                        if (diff < minY)
                            minY = diff;
                        if (diff > maxY)
                            maxY = diff;
                        ((_a = datas[isDiag])[_b = indData++] ?? (_a[_b] = [])).push([N, diff]);
                    }
                }
            }
        }
        ...
        fPlot.addLink(fPlot2);
        fPlot2.addLink(fPlot);
    };
})();
